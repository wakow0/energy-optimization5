...
for t in tqdm(range(1, len(df)), desc="Processing Fixed & Dynamic Solutions", unit="step"):
    for strategy, df_version in [("Fixed", df_fixed), ("Dynamic", df_dynamic)]:
        ...
        # Grid Export if excess renewable after charging or when discharge supports it
...
# Save only the required solution columns for both Fixed & Dynamic with "v2" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v2.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v2.csv", index=False)
