...
for t in tqdm(range(1, len(df)), desc="Processing Fixed & Dynamic Solutions", unit="step"):
...
# Final Validation Check for Constraints
def validate_constraints(df_version):
    # Check for violations
    violations = []
    
    for t in range(1, len(df_version)):
        # Check for energy balance
        available_renewable = df_version.loc[t, "dc_ground_1500vdc_power_output"] + df_version.loc[t, "windflow_33_[500kw]_power_output"]
        demand = df_version.loc[t, "ac_primary_load"]
        grid_import = df_version.loc[t, "P_import (kW)"]
        grid_export = df_version.loc[t, "P_export (kW)"]
        charge = df_version.loc[t, "P_bat_ch (kW)"]
        discharge = df_version.loc[t, "P_bat_dis (kW)"]
        soc = df_version.loc[t, "SOC (%)"]

        # Energy balance check
        if not np.isclose(demand, available_renewable + grid_import + discharge - grid_export - charge, atol=1e-2):
            violations.append(f"Energy balance violation at time step {t}")

        # SoC check
        if soc < soc_min or soc > soc_max:
            violations.append(f"SoC violation at time step {t}")

        # Mutual exclusivity checks (Import/Export and Charge/Discharge)
        if grid_import > 0 and grid_export > 0:
            violations.append(f"Import and Export violation at time step {t}")
        if charge > 0 and discharge > 0:
            violations.append(f"Charge and Discharge violation at time step {t}")
    
    return violations

# Run validation after processing all steps
violations = validate_constraints(df_fixed)
violations += validate_constraints(df_dynamic)
...
df_fixed.to_csv("generated_decision_variables_FIXED_v3.csv", index=False)
df_dynamic.to_csv("generated_decision_variables_DYNAMIC_v3.csv", index=False)

# Save only the required solution columns for both Fixed & Dynamic with "v3" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v3.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v3.csv", index=False)
