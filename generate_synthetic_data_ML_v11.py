...
for t in tqdm(range(1, len(df) - LOOK_AHEAD_WINDOW), desc="Processing Fixed & Dynamic Solutions (v11)", unit="step"):
    for strategy, df_version in [("Fixed", df_fixed), ("Dynamic", df_dynamic)]:
        ...
        max_power_change = 50  # Further reduced power jumps for smoother transitions
        if t > 1:
...
df_fixed.to_csv("generated_decision_variables_FIXED_v11.csv", index=False)
df_dynamic.to_csv("generated_decision_variables_DYNAMIC_v11.csv", index=False)

# Save only the required solution columns for both Fixed & Dynamic with "v11" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v11.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v11.csv", index=False)
