# Wind Turbine Predictive Maintenance Model for Wind Farm A
# Predicts failures 3–7 days ahead, SMOTE option, fixes issues
...
# Version check
print("Pipeline version: Wind Farm A, 2x sampling, April 21, 2025")
...
for pkg, version in required.items():
    try:
        if pkg_resources.get_distribution(pkg).version != version:
...
class DataLoader:
    ...
    def load_and_preprocess_data(self, max_files=20):
        ...
        if not unmatched.empty:
            ...
            print(f"Sampling normal data: {unmatched_samples} rows (2x anomaly count)")
...
class EnhancedTurbineFailurePredictor:
        ...
        self.models = {
            ...
            'lightgbm': Pipeline([
                ...
                ('classifier', LGBMClassifier(n_estimators=100, learning_rate=0.1, max_depth=6, 
                                              min_data_in_leaf=1, min_child_samples=20, num_leaves=6, 
    ...
    def train_and_evaluate(self, X, y):
        ...
        for name, model in self.models.items():
            ...
            try:
                ...
                print(f"Model {name} F1: {f1:.3f} (best so far: {best_f1:.3f})")
                if f1 >= best_f1 or (name == 'lightgbm' and abs(f1 - best_f1) < 0.001):
                    best_f1 = f1
                ...
                if hasattr(model.named_steps['classifier'], 'feature_importances_'):
                    self.feature_importances[name] = model.named_steps['classifier'].feature_importances_ / \
                                                    model.named_steps['classifier'].feature_importances_.sum()
    ...
    def save_feature_importance_summary(self, feature_names):
        ...
        print("\nFeature Importance Summary (Top 10 per model):")
...
class EnhancedMaintenanceScheduler:
    ...
    def generate_schedule(self, prediction_dict, timestamps, turbine_ids):
        ...
        schedule = []
        future_window = pd.Timedelta(days=7)
        min_window = pd.Timedelta(days=3)
        df = pd.DataFrame({'turbine_id': turbine_ids, 'timestamp': timestamps, 'probability': probabilities})
        df['timestamp'] = pd.to_datetime(df['timestamp'])
        df['week'] = df['timestamp'].dt.isocalendar().week
        grouped = df[df['probability'] >= 0.5].groupby(['turbine_id', 'week'])
        for (turbine_id, week), group in grouped:
            max_prob = group['probability'].max()
            if max_prob >= 0.5:
                priority = 'medium' if max_prob < 0.75 else 'high' if max_prob < 0.9 else 'critical'
                earliest_ts = group['timestamp'].min() + min_window
                schedule.append({
                    'turbine_id': turbine_id,
                    'probability': max_prob,
                    'priority': priority,
                    'maintenance_window': earliest_ts
                })
        schedule_df = pd.DataFrame(schedule)
        schedule_df = schedule_df.sort_values(by=['maintenance_window', 'probability'], ascending=[True, False])
...
def plot_maintenance_timeline(schedule_df):
    ...
    for priority in colors:
        ...
        plt.scatter(schedule_df[mask]['maintenance_window'], [1]*sum(mask), c=colors[priority], label=priority, s=100)
    plt.yticks([])
    plt.xlabel('Maintenance Window')
    ...
    print(schedule_df['priority'].value_counts().to_string())
...
if __name__ == "__main__":
    ...
    use_smote = False  # Set to True for SMOTE
    if use_smote:
    ...
    print("\nMaintenance Schedule (first 5 rows):\n", schedule_df.head().to_string(index=False))
