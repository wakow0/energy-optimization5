...
OUTPUT_FILE = "heuristic_optimization_results_v6.csv"
...
# Pricing thresholds
GRID_CHARGE_PRICE_THRESHOLD = 0.15
GRID_CHARGE_SOC_LIMIT = 0.6 * BATTERY_CAPACITY
...
# Initialize output columns with proper float types to avoid dtype warnings
df["P_bat_ch (kW)"] = 0.0
df["P_bat_dis (kW)"] = 0.0
df["P_import (kW)"] = 0.0
df["P_export (kW)"] = 0.0
...
for i in range(len(df)):
    ...
    # 1. Use renewable to meet demand
    net_demand = demand - renewable

    # 2. Discharge battery to meet remaining demand
    ...
    # 3. Import any remaining demand from grid
    if net_demand > 0:
    ...
    # 5. Grid charge if price is low and SoC is low
    if import_price < GRID_CHARGE_PRICE_THRESHOLD and SOC < GRID_CHARGE_SOC_LIMIT:
    ...
    # 6. Export any leftover renewable not used for demand or charging
...
print(f"✅ HOMER-style heuristic completed. Results saved to {OUTPUT_FILE}")
