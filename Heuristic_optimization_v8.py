...
OUTPUT_FILE = "heuristic_optimization_results_v8.csv"
...
for i in range(len(df)):
    ...
    # Inside your for-loop — REPLACES steps 5, 6, and 7
    # 5. Grid charge only if price is low AND renewables are not sufficient
    if import_price < GRID_CHARGE_PRICE_THRESHOLD and SOC < SOC_MAX and renewable < demand:
        grid_charge = min(500, (SOC_MAX - SOC) / CHARGE_EFFICIENCY)  # cap grid charge per step
        df.loc[i, "P_bat_ch (kW)"] += grid_charge
        df.loc[i, "P_import (kW)"] += grid_charge
        SOC += grid_charge * CHARGE_EFFICIENCY

    # 6. Discharge battery to export only if SoC is high (>70%)
    if sellback_price > import_price and SOC > 0.7 * BATTERY_CAPACITY:
    ...
    # 7. Export any unused renewables (after demand + battery charging)
