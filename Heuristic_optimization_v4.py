...
OUTPUT_FILE = "heuristic_optimization_results_v4.csv"
...
for i in range(len(df)):
    ...
    soc = max(SOC_MIN, min(soc, SOC_MAX))  # SOC should never exceed 100% or go below 5%
    df.loc[i, "SOC (%)"] = (soc / BATTERY_CAPACITY) * 100  # Save SOC as a percentage of battery capacity
...
print(f"Heuristic optimization Execution time: {end_time - start_time:.4f} seconds")