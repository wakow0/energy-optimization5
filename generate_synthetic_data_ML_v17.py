...
for t in tqdm(range(1, len(df) - LOOK_AHEAD_WINDOW), desc="Processing Fixed & Dynamic Solutions (v17)", unit="step"):
    for strategy, df_version in [("Fixed", df_fixed), ("Dynamic", df_dynamic)]:
        ...
        # Improved Battery Charging Rule - Wait for the lowest price before charging
        if encourage_charge:
            if future_import_prices < import_price:  # Future price is lower → Wait
                charge_power = 0  
            else:  # Future price is higher → Charge now
                charge_power = min(available_renewable, battery_max_charge)
        else:
            charge_power = 0

        # Improved Battery Discharge Rule - Wait for the highest price before discharging
        available_battery_energy = df_version.loc[t-1, "SOC (%)"] / 100 * battery_capacity
        if prioritize_export and df_version.loc[t-1, "SOC (%)"] > 20:
            if future_export_prices > export_price:  # Future price is higher → Wait
                discharge_power = 0  
            else:  # Future price is lower → Discharge now
                discharge_power = min(battery_max_discharge, available_battery_energy)
        else:
            discharge_power = min(battery_max_discharge, demand - available_renewable, available_battery_energy) if demand > available_renewable else 0

        # Ensure Energy Balance by Compensating for Shortfalls
        total_supply = available_renewable + discharge_power
        energy_shortfall = demand - total_supply
        grid_import = max(0, energy_shortfall) if not reduce_import else 0
        ...
        if charge_power > 0 and discharge_power > 0:
            if prioritize_export:
                charge_power = 0  # Prioritize discharge if selling power is profitable
            else:
                discharge_power = 0  # Otherwise, prioritize charging

        # Update SoC in percentage format with precision fix
        ...
        new_soc = np.round(prev_soc + soc_change, 2)  # Prevents floating-point errors
...
df_fixed.to_csv("generated_decision_variables_FIXED_v17.csv", index=False)
df_dynamic.to_csv("generated_decision_variables_DYNAMIC_v17.csv", index=False)

# Save only the required solution columns for both Fixed & Dynamic with "v17" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v17.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v17.csv", index=False)
...
validate_solution("solution_output_FIXED_v17.csv")
validate_solution("solution_output_DYNAMIC_v17.csv")
