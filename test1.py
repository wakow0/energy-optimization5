...
def plot_comparison(metric, ylabel, title, filename):
    ...
    ax.plot(solution_a["time"], solution_a[metric], linestyle="--", label=f"Solution A - {metric.capitalize()}", color="blue")
    ax.plot(solution_b_lr["time"], solution_b_lr[metric], linestyle="-", label=f"Solution B (LR) - {metric.capitalize()}", color="green")
    ax.plot(solution_b_xg["time"], solution_b_xg[metric], linestyle="-", label=f"Solution B (XG) - {metric.capitalize()}", color="red")
    ax.plot(solution_b_lstm["time"], solution_b_lstm[metric], linestyle="-", label=f"Solution B (LSTM) - {metric.capitalize()}", color="orange")

    # ✅ Labels and title
    ax.set_xlabel("Time")
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45)  # Rotate time labels for better visibility
    ...
    plt.show()
    plt.close(fig)  # Explicitly close the figure to prevent overlapping legends
