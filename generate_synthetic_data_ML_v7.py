...
# Smooth the data to remove spikes
window_size = 5  # Adjust the window size as needed
df["total_consumption_rate"] = df["total_consumption_rate"].rolling(window=window_size, min_periods=1, center=True).mean()
df["grid_sellback_rate"] = df["grid_sellback_rate"].rolling(window=window_size, min_periods=1, center=True).mean()
...
for t in tqdm(range(1, len(df) - LOOK_AHEAD_WINDOW), desc="Processing Fixed & Dynamic Solutions (v7)", unit="step"):
    for strategy, df_version in [("Fixed", df_fixed), ("Dynamic", df_dynamic)]:
        ...
        # Debug: Print energy balance for each time step
        energy_balance = demand - (grid_import + discharge_power - grid_export - charge_power)
        if not np.isclose(energy_balance, 0, atol=0.01):
            print(f"Energy balance violation at time step {t}: {energy_balance}")
            print(f"Demand: {demand}, Grid Import: {grid_import}, Discharge: {discharge_power}, Grid Export: {grid_export}, Charge: {charge_power}")
...
df_fixed.to_csv("generated_decision_variables_FIXED_v7.csv", index=False)
df_dynamic.to_csv("generated_decision_variables_DYNAMIC_v7.csv", index=False)

# Save only the required solution columns for both Fixed & Dynamic with "v7" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v7.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v7.csv", index=False)
...
def validate_solution(file_name):
    ...
    for t in range(1, len(df_check)):
        ...
        if not np.isclose(demand, 0, atol=0.01):  # Use a small tolerance for numerical precision
...
validate_solution("solution_output_FIXED_v7.csv")
validate_solution("solution_output_DYNAMIC_v7.csv")