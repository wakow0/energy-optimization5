...
for t in tqdm(range(1, len(df) - LOOK_AHEAD_WINDOW), desc="Processing Fixed & Dynamic Solutions (v18)", unit="step"):
    for strategy, df_version in [("Fixed", df_fixed), ("Dynamic", df_dynamic)]:
        ...
        # Ensure demand is met first before charging or exporting
        ...
        # Battery Discharge - Only if required for demand
        ...
        # Battery Charging - Only if excess energy exists
        ...
        # Grid Export - Only if additional energy is left
        ...
        df_version.loc[t, "ac_primary_load"] = demand  # ✅ Include ac_primary_load
...
df_fixed.to_csv("generated_decision_variables_FIXED_v18.csv", index=False)
df_dynamic.to_csv("generated_decision_variables_DYNAMIC_v18.csv", index=False)

# Save only the required solution columns for both Fixed & Dynamic with "v18" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)", "ac_primary_load"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v18.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v18.csv", index=False)
...
# Run validation
validate_solution("solution_output_FIXED_v18.csv")
validate_solution("solution_output_DYNAMIC_v18.csv")
