...
for t in tqdm(range(1, len(df)), desc="Processing Fixed & Dynamic Solutions", unit="step"):
    for strategy, df_version in [("Fixed", df_fixed), ("Dynamic", df_dynamic)]:
        ...
        # Charge if excess renewable is available
        charge_power = min(available_renewable, battery_max_charge)

        # Discharge if demand is higher than renewable supply
        discharge_power = min(battery_max_discharge, demand - available_renewable) if demand > available_renewable else 0

        # Grid Import if necessary
        grid_import = max(0, demand - available_renewable - discharge_power)
...
# Save only the required solution columns for both Fixed & Dynamic
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC.csv", index=False)
