...
for t in tqdm(range(1, len(df) - LOOK_AHEAD_WINDOW), desc="Processing Fixed & Dynamic Solutions (v9)", unit="step"):
    for strategy, df_version in [("Fixed", df_fixed), ("Dynamic", df_dynamic)]:
        ...
        # Implement Smooth Transitions to Prevent Spikes
        max_power_change = 100  # Limit power jumps between steps
        if t > 1:
...
df_fixed.to_csv("generated_decision_variables_FIXED_v9.csv", index=False)
df_dynamic.to_csv("generated_decision_variables_DYNAMIC_v9.csv", index=False)

# Save only the required solution columns for both Fixed & Dynamic with "v9" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v9.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v9.csv", index=False)
...
validate_solution("solution_output_FIXED_v9.csv")
validate_solution("solution_output_DYNAMIC_v9.csv")
