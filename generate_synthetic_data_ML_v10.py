...
for t in tqdm(range(1, len(df) - LOOK_AHEAD_WINDOW), desc="Processing Fixed & Dynamic Solutions (v10)", unit="step"):
...
df_fixed.to_csv("generated_decision_variables_FIXED_v10.csv", index=False)
df_dynamic.to_csv("generated_decision_variables_DYNAMIC_v10.csv", index=False)

# Save only the required solution columns for both Fixed & Dynamic with "v10" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v10.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v10.csv", index=False)
