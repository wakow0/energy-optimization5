...
def optimize_energy1(start_t, STRATEGY, prev_final_soc, prev_batch_powers):
    ...
    for t in range(start_t, end_t):
        ...
        model.addConstr(available_renewable + P_bat_dis[t] + P_import[t] == demand + P_bat_ch[t] + P_export[t])
        ...
        if t == start_t:
            ...
            model.addConstr(SOC[t] - prev_final_soc <= max_soc_change_per_batch)
            model.addConstr(prev_final_soc - SOC[t] <= max_soc_change_per_batch)
            for var_name, var_dict in zip(
                ["P_import", "P_export", "P_bat_ch", "P_bat_dis"],
                [P_import, P_export, P_bat_ch, P_bat_dis]
            ):
                model.addConstr(var_dict[t] - prev_batch_powers[var_name] <= max_power_change_per_batch)
                model.addConstr(prev_batch_powers[var_name] - var_dict[t] <= max_power_change_per_batch)
        else:
    ...
    if model.status != GRB.OPTIMAL:
        batch_times = df["time"].iloc[start_t:end_t].tolist()
        avg_demand = df["ac_primary_load"].iloc[start_t:end_t].mean()
        avg_renewable = (df["dc_ground_1500vdc_power_output"].iloc[start_t:end_t] +
                        df["windflow_33_[500kw]_power_output"].iloc[start_t:end_t]).mean()
        print(f"\n❌ Batch FAILED at interval {start_t} ({df['time'].iloc[start_t]})")
        print(f"   ➤ prev_final_soc: {prev_final_soc:.2f}")
        print(f"   ➤ Average demand: {avg_demand:.2f} kW")
        print(f"   ➤ Average renewables: {avg_renewable:.2f} kW")
        print(f"   ➤ Batch times: {batch_times[0]} to {batch_times[-1]}")
        

        with open("failure_log.csv", "a", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([
                start_t,
                df["time"].iloc[start_t],
                prev_final_soc,
                avg_demand,
                avg_renewable,
                batch_times[0],
    ...
    prev_final_soc = max(SOC[end_t - 1].X, soc_min)
...
def optimize_energy2(start_t, STRATEGY, prev_final_soc, prev_batch_powers):
    model = Model("Energy_Optimization")
    model.Params.OutputFlag = 0  # Suppress solver output

    end_t = min(start_t + BATCH_SIZE, time_intervals)
    
    SOC_BUFFER = 0.95 * soc_max  # ✅ Apply SOC Buffer (Max 95%)
    min_discharge_value = 50  # ✅ Ensure minimum discharge when SOC is high

    P_import, P_export, P_bat_ch, P_bat_dis, SOC = {}, {}, {}, {}, {}
    X_import, X_export, X_bat_ch, X_bat_dis, X_curtail = {}, {}, {}, {}, {}
    P_curtail = {}
    ...
    for t in range(start_t, end_t):
        ...
        # ✅ Introduce Curtailment Variable
        P_curtail[t] = model.addVar(lb=0, name=f"P_curtail_{t}")
        X_curtail[t] = model.addVar(vtype=GRB.BINARY, name=f"X_curtail_{t}")
        
        # ✅ Ensure Mutual Exclusivity: Grid Import vs Curtailment
        model.addConstr(P_import[t] <= inverter_capacity * (1 - X_curtail[t]))
        model.addConstr(P_curtail[t] <= max_renewable_capacity * X_curtail[t])
        
        # ✅ Ensure Minimum Discharge at High SOC
        model.addConstr(P_bat_dis[t] >= min_discharge_value * (SOC[t] >= SOC_BUFFER))
    
    # ✅ Updated Energy Balance Constraint
    for t in range(start_t, end_t):
        available_renewable = df["dc_ground_1500vdc_power_output"].iloc[t] + df["windflow_33_[500kw]_power_output"].iloc[t]
        demand = df["ac_primary_load"].iloc[t]
    ...
    # ✅ Objective: Minimize Cost + Penalize Curtailment
    penalty_cost = 1000  # High enough to discourage curtailment
    model.setObjective(
        quicksum(P_import[t] * (FIXED_IMPORT_PRICE if STRATEGY == "FIXED" else df["total_consumption_rate"].iloc[t]) -
                 P_export[t] * (FIXED_EXPORT_PRICE if STRATEGY == "FIXED" else df["grid_sellback_rate"].iloc[t]) +
    ...
    if model.status != GRB.OPTIMAL:
        print(f"❌ Warning: Solver did not find an optimal solution in batch starting at interval {start_t}.")
        return None, prev_final_soc, prev_batch_powers
    ...
    return results, prev_final_soc, prev_batch_powers
