...
for t in tqdm(range(1, len(df) - LOOK_AHEAD_WINDOW), desc="Processing Fixed & Dynamic Solutions (v8)", unit="step"):
    for strategy, df_version in [("Fixed", df_fixed), ("Dynamic", df_dynamic)]:
        ...
        else:  # Dynamic Pricing with refined adjustments
        ...
        # Refined Dynamic Pricing Strategy for Better Performance
        ...
        # Optimized Battery Export Strategy
        ...
        # Strict Energy Balance Enforcement
        ...
        # Round values to prevent floating-point mismatches
        grid_import = np.round(grid_import, 3)
        grid_export = np.round(grid_export, 3)
        charge_power = np.round(charge_power, 3)
        discharge_power = np.round(discharge_power, 3)
...
df_fixed.to_csv("generated_decision_variables_FIXED_v8.csv", index=False)
df_dynamic.to_csv("generated_decision_variables_DYNAMIC_v8.csv", index=False)

# Save only the required solution columns for both Fixed & Dynamic with "v8" naming
solution_columns = ["time", "P_import (kW)", "P_export (kW)", "P_bat_ch (kW)", "P_bat_dis (kW)", "SOC (%)"]

df_fixed[solution_columns].to_csv("solution_output_FIXED_v8.csv", index=False)
df_dynamic[solution_columns].to_csv("solution_output_DYNAMIC_v8.csv", index=False)
...
validate_solution("solution_output_FIXED_v8.csv")
validate_solution("solution_output_DYNAMIC_v8.csv")
